  ...
  return async function (ctx, next) {
    if (ctx.path.indexOf('/weapp/') === 0) {
      ...
      if (!checkWhiteList(ctx, parentWhiteLists) && !whiteLists.includes(ctx.request.path)) {
        ...
        const { deviceid, sign } = ctx.request.header;
        ...
        if (token) {
        ...
        } else if (deviceid && sign) {
          const devices = await ctx.model.Devices.findOne({
            where: {
              id: deviceid
            },
            include: [{
          ...
          const secret = sha1(`${deviceid}${devices && devices.secret ? devices.secret : ''}`);
          if (devices && sign === secret) {
            ctx.state.devices = devices;
            let roleIds = devices.roles.map(item => item.id);
            let menus = await ctx.model["RoleMenu"].findAll({
              where: {
                roleId: {
                  [Op.or]: roleIds
