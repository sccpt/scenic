...
class WeappPayService extends Service {
        ...
        try {
          ...
          if (xml && xml.return_code == 'SUCCESS') {
            ...
            if (xml.sign == sign && xml.out_trade_no) {
              const orderDatas = await app.model.TicketsOrders.weappGetMoreById(xml.out_trade_no);
              const totalPrice = wxpayUitls.getmoney(orderDatas.price);
              // 验证金额
              if (orderDatas && totalPrice == xml.total_fee) {
                const res = await app.model.TicketsOrders.weappUpdate({ ids: orderDatas.ids, pay_id: xml.transaction_id, pay_money: xml.total_fee });
                if (res) {
                  resolve(true);
                }
                reject(false);
  ...
  async refund(id) {
    const { ctx, app } = this;
    const datas = await app.model.TicketsOrders.findByPk(id);
    if (datas.pay_status == 1) {
      // 推送消息
      await ctx.service[app.config.public].admin.system.notice.send('pay_refund', {
        title: '发起退款',
        content: `订单号:${datas.order_id}取消购票，已发起退款，请注意退款状态！`,
        createdAt: new Date(),
      });
      const params = {
        appid: app.config.weapp.appId,
      ...
      const { xml } = await wxpayUitls.parserXML(res.data.toString());
      if (xml && xml.return_code == 'SUCCESS') {
        await app.model.TicketsOrders.update({ refund_status: 1 }, { where: { id } });
      }
    }
  }
}
